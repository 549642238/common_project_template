...
int main(void)
{
	...
	if (!bitmap) {
	      MSG(2, "%s", strerror(errno));
	      return 0;
	}
	MSG(0, "Construct bitmap success[%d]\n", ELE_NUM);

	MSG(0, "Set bit (mod 2 = 0)\n");
	for (pos = 0; pos < ELE_NUM; pos += 2) {
		set_bit(bitmap, pos);
	}

	MSG(0, "Clear bit (mod 3 = 0)\n");
	for (pos = 0; pos < ELE_NUM; pos += 3) {
		clear_bit(bitmap, pos);
	}

	MSG(0, "Test and set bit (mod 6 = 0)\n");
	...
	MSG(0, "Test and clear bit (mod 12 = 0)\n");
	...
	for (pos = 0; pos < ELE_NUM; ++pos) {
		MSG(0, "%d", test_bit(bitmap, pos) ? 1: 0);
	}
	MSG(0, "\n");
	MSG(0, "set_6 %d, clear_12 %d\n", set_6, clear_12);

	if ((pos = find_next_bit(bitmap, ELE_NUM, 0)) < ELE_NUM)
		MSG(0, "Next set bit %u\n", pos);
	if ((pos = find_next_zero_bit(bitmap, ELE_NUM, 0)) < ELE_NUM)
		MSG(0, "Next zero bit %u\n", pos);
